      ...
      >
        <nav class="panel">
          <div class="panel-heading">
            <div class="columns">
              ...
              <div class="column is-1 is-offset-7">
                <div @click="minimizeEditor" style="cursor: pointer">
                  {{ editorMinimized ? "➕" : "➖" }}
                </div>
              </div>
            </div>
          ...
          <p class="panel-tabs">
            ...
            >
            <a
              :class="{ 'is-active': activeTab === 'bubbles' }"
              @click="openTab('bubbles')"
              >{{ $t("menu.bubbles") }}</a
            >
            <a
              :class="{ 'is-active': activeTab === 'logo' }"
              @click="openTab('logo')"
              >{{ $t("menu.logo") }}</a
            >
            <a
              :class="{ 'is-active': activeTab === 'imp-exp' }"
              @click="openTab('imp-exp')"
              >{{ $t("menu.importExport") }}</a
            >
<!--            
            <v-select v-bind:options="languages" v-model="selectedLanguage"  @option:selected="switchLocale($event)">
            </v-select> -->
            <!-- <v-select @change="" :options="languages" label="select">
  <template slot="option" slot-scope="option">
      <img id="imgLang" :src="option.logo" />
      {{ option.name }}
  </template>
</v-select> -->
        ...
        <div
          ...
          :style="{ display: editorMinimized ? 'none' : null }"
        >
            ...
            <div class="field">
              ...
              <div class="controle">
                <input
                  ...
                  type="color"
                  placeholder="#2d"
                  v-model="param.progressColor"
                  id="progresscolorInput"
                  style="width: 100%"
                />
              </div>
            </div>
          ...
          <div v-if="activeTab === 'bubbles'" data-name="bubbles-tab">
                    ...
                    <div class="columns">
                      ...
                      >
                        <span
                          ...
                          v-if="
                            option.bubbleCount > 0 && option.bubbleCount < 3
                          "
                          style="font-size: 9px"
                          >{{ $t("notEnoughBubbles") }}</span
                        >
                        <span
                          class="tag is-success"
                          style="font-size: 9px"
                          v-else-if="option.bubbleCount >= 3"
                          >{{ $t("valide") }}</span
                        >
                        <span
                          class="tag is-light"
            ...
            <draggable
              tag="div"
              :list="bubbles"
              class="bubbles-editor-section"
              handle=".bubble-drag-handle"
            >
              <bubbleEdit
                style="margin: 10px"
                v-for="(bubble, i) in bubbles"
                @onDeleteBubble="deleteBubble(i)"
                :key="i"
                :index="i"
                :bubble="bubble"
              />
            ...
            <div style="text-align: center; margin-top: 10px">
              <div
                class="alert-warning"
                v-if="bubbles.length < 3"
                style="font-size: 9px; margin-bottom: 10px"
              >
                {{ $t("warning") }} ({{ 3 - bubbles.length }})
                <u style="cursor: pointer" @click="createNeededBubbles"
                  >({{ $t("createBubble") }})</u
                >
              </div>
              <button
          ...
          <div style="text-align: center">
            <div class="row">
              <button class="button is-primary" @click="simulateLoading">
                {{ $t("replayAnim") }}
                <i class="fa fa-arrows-rotate" style="margin-left: 5px"></i>
              </button>
            </div>
          </div>
        </div>
      ...
      <vue-topprogress
        ref="topProgress"
        :color="param.progressColor"
        :height="5"
      ></vue-topprogress>
      <div
        :class="{ slideup: param.showBubbles }"
        style="
          width: 100%;
          height: 25%;
          display: flex;
      ...
      <div
        class="bull-container"
        :class="{ 'popup-animate': param.showBubbles }"
      >
        <template v-if="param.showBubbles">
          <!--@click.native="onSelectBubble(bubble, b_i)"-->
          <LoginBubble
            v-for="(bubble, b_i) in bubbles"
            :class="{ bubble: param.playAnimation }"
            :opacity-percentage="bubble.opacity"
            :size="bubble.size"
            :style="bubble.style"
            :key="b_i"
          >
            <template v-for="(line, i) in bubble.content">
              <span
                :key="i"
                :style="{
                  'font-size': computeBullFontSize(bubble.size),
                  ...line.style,
                }"
                >{{ line.label }}</span
              >
      ...
      <div style="width: 100%; height: 20%; display: flex">
        ...
        >
          <div
            style="display: flex"
            :style="{ height: '50%', ...param.logo.style }"
          >
            <img
              v-if="param.logo.src"
              :src="param.logo.src"
...
export default {
  ...
  methods: {
    ...
    bubbleCount(lang) {
      if (!this.param.bubbles[lang]) return 0;
      else return this.param.bubbles[lang].length;
    },
    openTab(code) {
      this.activeTab = code;
    ...
    importedParam(param) {
      ...
      if (param.logo && param.logo.src) this.logoFileName = null;
    ...
    onEditorActivated() {
      if (this.editorMinimized) this.minimizeEditor();
    },
    onEditorDeactivated() {
      if (!this.editorMinimized) this.minimizeEditor();
    },
    ...
    showProgress(value) {
      ...
      if (this.param.showBubbles) this.showBubble();
    },
    showBubble() {
      // get the index of the bubble to show
      const current_bubble_index = Math.floor(
        (this.startingProgress - (100 - this.progress)) / this.bubbleProgress
      );
      if (current_bubble_index >= this.bubbles.length) return;
    ...
    newStep(progress, dfd, msg) {
      ...
      if (sessionStorage.getItem("hideMenu") == "true") return;
      this.Dfds.push(dfd);
    ...
    this.$watch(
      () => {
        if (this.$refs.topProgress) {
          return this.$refs.topProgress.progress;
        }
      },
      (value) => {
